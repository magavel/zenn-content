...
本記事では、ログ調査のコストを大幅に削減するためのDatadogとAmazon Athenaの効果的な使い分けについて、実際のコスト試算や注意点とあわせてご紹介します。

特に、長期間のログを対象とした調査におけるコスト問題を、Athenaを活用することで解決した事例として参考になれば幸いです。
...
詳細は伏せますが、具体的な事例として、警察から特定のユーザーに関する調査協力依頼がありました。この依頼では、複数のユーザーIDについて、過去1年間のアクセスログを抽出し、提出する必要がありました。
...
このように、Athenaを利用することで、**今回のケースではコストを約$1552から約$32へと、約98%削減できることがわかりました。**
...
今回の取り組みにあたり、ご協力いただいたSREの[@takehitohito](https://zenn.dev/takehitohito)さん、VPoTの[@rinchsan](https://zenn.dev/rinchsan)さん、そしてCS-Riskチームの[@puku]()さんに感謝します。ありがとうございました！
